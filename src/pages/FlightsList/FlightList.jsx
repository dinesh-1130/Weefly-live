	...
	useEffect(() => {
		...
		if (location.state) {
			...
			if (stateTripType === 'One Way' && stateOneWay) {
				...
				console.log(
					'Setting flights from flightsData:',
					stateFlightsData
				)
			...
			if (stateFightSearchData) {
				...
				setSearchdata({
					...
					to: destinationValue,
					flightDepatureDate:
						stateFightSearchData.dateOfSearch || null,
					flightReturnDate:
						stateFightSearchData.returnDateOfSearch || null,
					travelClass: stateFightSearchData.travelClass || 'Economy',
				})
			}
		} else {
			console.log(
				'No location.state found - this might be a direct page load'
			)
		}
							...
							return allFlights
								.map(flight => {
									try {
										...
										const departureTime =
											segment.DepartDate &&
											Array.isArray(segment.DepartDate)
												? segment.DepartDate[0]?.split(
														'-'
												  )[1] || 'N/A'
												: 'N/A'
										...
										const arrivalTime =
											segment.ArriveDate &&
											Array.isArray(segment.ArriveDate)
												? segment.ArriveDate[0]?.split(
														'-'
												  )[1] || 'N/A'
												: 'N/A'
										...
										const duration =
											segment.Duration &&
											Array.isArray(segment.Duration)
												? Math.round(
														(Number(
															segment.Duration[0]
														) || 0) / 60
							...
							const outwardFlights = outwardList
								.map(flight => {
									try {
										...
										console.error(
											'Error processing outward flight:',
											error
										)
										return null
									}
							...
							const returnFlights = returnList
								.map(flight => {
									try {
										...
										console.error(
											'Error processing return flight:',
											error
										)
										return null
									}
