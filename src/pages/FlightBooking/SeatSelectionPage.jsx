...
export default function SeatSelection() {
	...
	const convertLuggageOption = async () => {
		const LuggageOptions = parseLuggageOptions(
		...
		customParams.forEach(param => {
			if (param.Name === 'DateOfBirth' && param.Value) {
				...
				if (parts.length === 3) {
					...
					console.warn(
						'DateOfBirth is not in yyyy-mm-dd format:',
						param.Value
					)
				}
			}
	...
	const handleProcessTerm = async e => {
		...
		if (response.ok) {
			...
			if (contentType && contentType.includes('application/json')) {
				...
				try {
					const priceList =
						res.data.Router[0].GroupList[0].Group[0].Price
					...
					if (
						...
						navigate('/booking/payment', {
							state: {
								...
								Address:
									travellerDetails.BillingDetails.Address,
									...
									{activeTab === 'Extra luggages' && (
										<div className='w-full'>
											...
											<div className='grid grid-cols-1 sm:grid-cols-2 gap-4 w-full overflow-auto h-[400px] no-scrollbar'>
												{convertedLuggageOptions.map(
													(Luggage, index) => {
														const isSelected =
															selectedLuggage.some(
																l =>
																	l ===
																	Luggage
															)
