		...
		try {
			...
			if (!commissionDetails) {
				...
				if (Tax && Commission) {
					...
					if (
						commissionDetails.CommissionType.toLowerCase() ===
						'percentage'
					) {
						setTax(`${Tax}%`)
						const taxAmount =
							(location.state?.convertedPrice * Tax) / 100
						const commissionAmount =
							(location.state?.convertedPrice * Commission) / 100
						const totalAmount =
							location.state?.convertedPrice +
							taxAmount +
							commissionAmount
							...
							<div className="p-4 space-y-3 text-[14px] text-black font-['Lato']">
								...
								{location.state?.seatCharge && (
									<div className='flex justify-between'>
										...
										<span className='font-semibold flex gap-1 items-end'>
